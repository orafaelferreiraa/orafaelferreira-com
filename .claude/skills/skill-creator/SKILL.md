...
Beyond `name`/`description`, Claude Code recognizes these optional frontmatter
fields — use them when a skill needs more than "load knowledge when relevant":

| Field                      | Use it for                                                          |
| -------------------------- | -------------------------------------------------------------------- |
| `disable-model-invocation` | Action skills with side effects (`/deploy`, `/commit`) — only a user should trigger them, not Claude on its own |
| `user-invocable`           | Background context skills nobody should run as a command (`false` hides it from `/`) |
| `allowed-tools` / `disallowed-tools` | Pre-approve or block specific tools for the turn that invokes the skill |
| `context: fork` + `agent`  | Run the skill in an isolated subagent instead of the main conversation |
| `model` / `effort`         | Override the model or reasoning effort while the skill is active   |
| `paths`                    | Restrict auto-loading to files matching a glob (monorepo package skills) |

These are Claude-Code-specific; they're ignored by other Agent-Skills-standard
tools, so adding them doesn't break portability to Copilot or other clients.
...
4. **Create** — Write SKILL.md in `.claude/skills/<skill-name>/`
...
After creating the skill in `.claude/skills/`, create a symlink in the appropriate category:

```bash
# Pattern: skills/<language>/<category>/<short-name> -> ../../../.claude/skills/<full-skill-name>

# Example for azure-ai-agents-py in python/foundry:
cd skills/python/foundry
ln -s ../../../.claude/skills/azure-ai-agents-py agents

# Example for azure-cosmos-db-py in python/data:
cd skills/python/data
ln -s ../../../.claude/skills/azure-cosmos-db-py cosmos-db
```
...
# Should show: agents -> ../../../.claude/skills/azure-ai-agents-py
```
...
- [ ] Skill created in `.claude/skills/<skill-name>/`
- [ ] Symlink created in `skills/<language>/<category>/<short-name>`
- [ ] Symlink points to `../../../.claude/skills/<skill-name>`

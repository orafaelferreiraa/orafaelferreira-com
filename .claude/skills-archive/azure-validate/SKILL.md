...
> **Resolving MCP tools in Claude Code**
>
> This skill was ported from GitHub Copilot's Azure MCP tool pack. Tool names
> throughout this file and its `references/` (`mcp_azure_mcp_*`, `mcp_bicep_*`,
> `azure__role`, ...) are Copilot-era identifiers and won't exist verbatim in
> Claude Code. When you hit one:
> 1. Run `ToolSearch` with the described capability (e.g.
>    `ToolSearch("azure subscription list")`, `ToolSearch("bicep best practices")`)
>    to find the real `mcp__<server>__<tool>` name for whatever Azure/Bicep MCP
>    server is connected in this session.
> 2. If no matching MCP tool is connected, fall back to the equivalent `az`,
>    `azd`, or `bicep` CLI command via Bash — every capability referenced here
>    also has a direct CLI equivalent.

...
E mudou muita coisa. Em pouco mais de quatorze meses o Core saltou de \`v1.12\` para \`v1.16\`, o provider \`azurerm\` teve uma major version com quebra real, o Stacks saiu de beta, a HashiCorp virou parte da IBM, e apareceu uma linguagem nova de governança.

Nada disso invalida o certificado, que vale até **26/07/2027**. Mas é tudo conteúdo novo para quem quer continuar afiado com Terraform.

| Item | Situação |
|---|---|
| Certificação emitida | 26/07/2025 |
| Válida até | 26/07/2027 |
| Core na data da certificação | ~v1.12 |
| Core hoje | v1.16.3 |
...
Esse é o conceito mais importante do período inteiro. A base nasceu na 1.10 e amadureceu na 1.11.
...
O Core é o motor. O \`azurerm\` é o que a gente escreve toda hora. Na data da minha certificação o provider estava na **v4.37.0**. De lá até hoje foram dezenas de minors na linha 4.x, depois a 5.0, e hoje a linha corrente é a **5.6.0** (17/09/2026).
...
Vale o registro: **não houve outra major version entre a 4.0 (agosto de 2024) e a 5.0**. A linha 4.x foi inteira em minors.
...
Além de ephemeral values e write-only arguments, dois outros conceitos ganharam peso no período:

**Custom conditions.** É uma forma de fazer o Terraform recusar um \`plan\` ou \`apply\` **antes** de bater na API do provider, com uma mensagem de erro escrita por você em vez do erro genérico da cloud. O \`validation\` vive dentro de uma \`variable\`; \`precondition\` e \`postcondition\` vivem dentro de um bloco \`lifecycle\` de resource, data source ou output:
...
O esqueleto do Terraform que eu estudei continua de pé, mas três coisas mudaram o suficiente para entrar na minha lista de prática:

- **Ephemeral values e write-only arguments**, porque mudam como segredo trafega.
- **Stacks e tfpolicy**, que juntos cobrem o espaço que hoje é preenchido por wrapper e por linguagem de política separada.

O resto (backend, módulo, state, import, workspace) é o mesmo Terraform que eu estudei.

...
Fui atrás, release por release, do que mudou desde então. Foram três releases do Core (v1.35, v1.36 e v1.37) em nove meses, mais uma mudança grande no ecossistema de Ingress que pesa direto no dia a dia de quem administra cluster.
...
## Três releases: 1.35, 1.36 e 1.37

### v1.35 "Timbernetes" — 17/12/2025

60 enhancements (17 stable, 19 beta, 22 alpha). O destaque prático:

- **In-place Pod resource updates** chega a GA, permitindo ajustar CPU e memória de um Pod **sem recriá-lo**. Quem já teve que reiniciar um StatefulSet inteiro para subir o limite de memória de um container sabe o tamanho disso.
...
É o mesmo raciocínio que a **ValidatingAdmissionPolicy** já tinha trazido para validação, agora estendido para mutação. Na prática, onde hoje existe um webhook mutador em produção (injeção de sidecar de service mesh é o caso clássico), passa a existir uma alternativa nativa sem processo externo para manter no ar.

## Traduzindo: a família DRA

A **Dynamic Resource Allocation** já era GA na v1.34, o mecanismo por trás de scheduling de GPU e TPU deixou de ser experimental exatamente ali. O que chegou depois foram as peças que faltavam para ela ser útil em produção de verdade.
...
De tudo que mudou no período, essa é a que mais pesa para quem administra cluster, não porque uma regra específica mudou, mas porque o controller que praticamente todo mundo usava para praticar Ingress **deixou de existir como projeto ativo**.
...
Na prática, quem monta um cluster hoje já não tem mais a rota mais óbvia disponível, e a **Gateway API virou a primeira opção**, não mais a alternativa.
...
Quatro movimentos do período que ainda não viraram feature estável, mas dizem para onde o projeto está indo.
...
A CKA vale até **23/12/2027** e nada aqui invalida o certificado, recertificar não é necessário por causa de nenhuma dessas mudanças. Mas se o objetivo é continuar afiado, o levantamento aponta dois pontos:

- **A família DRA**. Device management para GPU e cargas de IA é onde o projeto está concentrando esforço de verdade. Vale montar um cenário com \`DeviceTaintRule\` e entender o ciclo de \`ResourceClaim\`.
- **A migração definitiva de Ingress para Gateway API**. O que mudou foi qual dos dois ainda existe como implementação de referência para praticar.

Todo o resto (troubleshooting, kubeadm, RBAC, NetworkPolicy, storage) continua exatamente como estava.

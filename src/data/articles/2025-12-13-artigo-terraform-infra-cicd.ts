...
Nesse projeto apliquei em prática o tfdocs. O \`infra-deploy.yml\` roda **terraform-docs** e injeta a referência do módulo no \`README.md\`. Se não houver mudanças, nenhum commit é feito, a documentação é altamente flexivel.
...
3. **Push do app** (ou workflow_run de infra) → roda \`deploy-app.yml\`: builda e publica a pasta \`dist/\` no SWA. Esse foi o momento mais satisfatório do projeto: ver o blog ir ao ar segundos depois do merge.
